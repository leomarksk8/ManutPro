...
export default function RelatorioPreventiva() {
    ...
    return (
      <div className="bg-slate-100 p-4 md:p-8 min-h-screen">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-2xl font-bold text-slate-800 mb-6">Relatório de Preventiva</h1>
  ...
  const liberacoesFiltradas = liberacoes.filter((l) => {
    ...
    if (l.tipo_manutencao) {
      return l.tipo_manutencao === 'preventiva';
    }
    ...
    return equipamento && equipamento.tipo_manutencao === 'preventiva';
  });

  const equipamentosParados = activeTurnoForReport ?
    equipamentosParaUsar.filter((e) => {
      if (e.status === 'concluida' || e.tipo_manutencao !== 'preventiva') return false;
  ...
  return (
    <>
      ...
      <div className="bg-slate-100 p-4 md:p-8 min-h-screen">
        <div className="max-w-6xl mx-auto">
          <div className="no-print mb-4 md:mb-6 space-y-3 md:space-y-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-0">
              <h1 className="text-xl md:text-2xl font-bold text-slate-800">Relatório de Preventiva</h1>
              ...
              <>
                <div className="flex justify-between items-start mb-8 border-b-2 border-slate-300 pb-6">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-blue-700 mt-2">MANUTENÇÃO PREVENTIVA</p>
                ...
                {liberacoesFiltradas.length === 0 && equipamentosParados.length === 0 ? (
                  ...
                  <>
                              ...
                              <div key={index} className="border-l-4 border-green-600 pl-4">
                                    ...
                                    <Button
                                      onClick={() => {
                                        ...
                                        setFormEdicao({
                                          ...
                                          tipo_preventiva: lib.tipo_preventiva || 'total'
                                        });
                                      }}
                                      variant="outline"
                                      size="sm"
                                ...
                                {lib.ordem_manutencao && (
                                  <p className="text-sm text-slate-700 mb-2">
                                    <strong>OM PRINCIPAL:</strong> {lib.ordem_manutencao}
                                  </p>
                                )}
                                ...
                                <p className="text-sm font-semibold text-slate-900 mb-1">
                                  TIPO: <span className={lib.tipo_preventiva === 'total' ? 'text-green-700' : 'text-blue-700'}>
                                ...
                                {lib.oms_realizadas && lib.oms_realizadas.length > 0 && (
                                  <>
                                    <p className="text-sm font-semibold text-slate-900 mb-1">OMs REALIZADAS NO TURNO {lib.turno}:</p>
                                    <div className="bg-green-50 border border-green-200 rounded p-3 mb-2">
                                      {lib.oms_realizadas.map((om, idx) => (
                                        <div key={idx} className="mb-2 last:mb-0">
                                          <p className="text-sm text-green-800">
                                            • <strong>OM {om.numero_om}</strong> ({om.tipo_om}): {decodeHtmlEntities(om.descricao)}
                                          </p>
                                        </div>
                                      ))}
                                    </div>
                                  ...
                                  if (historicoAtividades.length > 0) {
                                    return (
                                      <div className="bg-blue-50 border border-blue-200 rounded p-3 mb-2">
                                        <p className="text-sm font-bold text-blue-900 mb-2">ATIVIDADES REALIZADAS EM TURNOS ANTERIORES:</p>
                                ...
                                {lib.tipo_preventiva === 'parcial' && lib.atividades_nao_realizadas && lib.atividades_nao_realizadas.length > 0 && (
                                  <div className="bg-blue-50 border-2 border-blue-300 rounded p-3 mb-2">
                                    <p className="text-sm font-bold text-blue-900 mb-2">ATIVIDADES NÃO REALIZADAS:</p>
                                    {lib.atividades_nao_realizadas.map((ativ, idx) => (
                                      <div key={idx} className="mb-3 pb-3 border-b border-blue-200 last:border-0">
                                        {ativ.om && (
                                          <p className="text-sm text-blue-800">
                                            <strong>OM:</strong> {ativ.om}
                                          </p>
                                        )}
                                        <p className="text-sm text-blue-800">
                                          <strong>ATIVIDADE:</strong> {ativ.atividade}
                                        </p>
                                        <p className="text-sm text-blue-800">
                                          <strong>MOTIVO:</strong> {ativ.motivo}
                                        </p>
                                        <p className="text-sm text-blue-800">
                                          <strong>RECOMENDAÇÃO:</strong> {ativ.recomendacao?.toUpperCase()}
                                        </p>
                                      </div>
                                    ))}
                                  </div>
                                ...
                                {(() => {
                                  const equipamentoReal = equipamentos.find((e) => e.id === lib.equipamento_id);
                                  const observacoes = lib.observacoes || equipamentoReal?.anotacoes;
                    ...
                    {equipamentosParados.length > 0 && (
                      <div className="mb-8">
                        <h2 className="text-2xl font-bold text-blue-700 mb-6">EQUIPAMENTOS EM MANUTENÇÃO PREVENTIVA</h2>
                        <div className="space-y-4">
                          {ordenarPorPrioridadeFrota(equipamentosParados, 'codigo').map((equip, index) => (
                            <div key={index} className="border-l-4 border-blue-600 pl-4">
                              ...
                              <p className="text-sm font-semibold text-slate-900 mb-2 mt-4">ATIVIDADES:</p>

                              {equip.oms_preventiva && equip.oms_preventiva.length > 0 && (
                                <div className="space-y-3">
                                  {equip.oms_preventiva.filter((om) => om.status === 'REALIZADO' || om.status === 'REALIZADO_TURNO_ATUAL').length > 0 && (
                                    <div className="bg-green-50 border border-green-200 rounded p-3">
                                      <p className="text-sm font-bold text-green-900 mb-2">OMs REALIZADAS:</p>
                                      {equip.oms_preventiva
                                        .filter((om) => om.status === 'REALIZADO' || om.status === 'REALIZADO_TURNO_ATUAL')
                                        .map((om, idx) => {
                                          const historicoOm = equip.historico_oms_por_turno?.find((hist) =>
                                            hist.oms_realizadas?.some((realizada) => realizada.numero_om === om.numero_om)
                                          );

                                          const turnoRealizou = historicoOm ? historicoOm.turno : turnoAtivo ? turnoAtivo.letra : null;
                                          const supervisorRealizou = historicoOm ? historicoOm.supervisor : turnoAtivo ? turnoAtivo.supervisor : null;
                                          const tecnicoRealizou = historicoOm ? historicoOm.tecnico_lider : turnoAtivo ? turnoAtivo.tecnicos_lideres : null;
                                          ...
                                          return (
                                            <div key={idx} className="mb-2 last:mb-0">
                                              ...
                                              {turnoRealizou && (
                                                <p className="text-xs text-green-700 ml-4 font-semibold">
                                                  Turno {turnoRealizou} - {tecnicoRealizou || supervisorRealizou || 'N/A'}
                                                </p>
                                              )}
                                            </div>
                                          );
                                  ...
                                  {equip.oms_preventiva.filter((om) => om.status === 'PENDENTE' || !om.status).length > 0 && (
                                    <div className="bg-orange-50 border border-orange-200 rounded p-3">
                                      <p className="text-sm font-bold text-orange-900 mb-2">OMs PENDENTES:</p>
                                      {equip.oms_preventiva
                                        .filter((om) => om.status === 'PENDENTE' || !om.status)
                                        .map((om, idx) => (
                                          <div key={idx} className="mb-2 last:mb-0">
                                  ...
                                  {equip.oms_preventiva.filter((om) => om.status === 'NAO_REALIZADO').length > 0 && (
                                    <div className="bg-red-50 border border-red-200 rounded p-3">
                                      <p className="text-sm font-bold text-red-900 mb-2">OMs NÃO REALIZADAS:</p>
                                      {equip.oms_preventiva
                                        .filter((om) => om.status === 'NAO_REALIZADO')
                                        .map((om, idx) => {
                                          const historicoOm = equip.historico_oms_por_turno?.find((hist) =>
                                            hist.oms_nao_realizadas?.some((naoRealizada) => naoRealizada.numero_om === om.numero_om)
                                          );

                                          const turnoNaoRealizou = historicoOm ? historicoOm.turno : turnoAtivo ? turnoAtivo.letra : null;
                                          const supervisorNaoRealizou = historicoOm ? historicoOm.supervisor : turnoAtivo ? turnoAtivo.supervisor : null;
                                          const tecnicoNaoRealizou = historicoOm ? historicoOm.tecnico_lider : turnoAtivo ? turnoAtivo.tecnicos_lideres : null;
                                          ...
                                          return (
                                            <div key={idx} className="mb-3 pb-3 border-b border-red-200 last:border-0">
                                              ...
                                              {om.motivo_nao_realizada && (
                                                <p className="text-xs text-red-700 ml-4 mt-1">
                                                  <strong>Motivo:</strong> {om.motivo_nao_realizada}
                                                </p>
                                              )}
                                              {om.recomendacao_nao_realizada && (
                                                <p className="text-xs text-red-700 ml-4">
                                                  <strong>Recomendação:</strong> {om.recomendacao_nao_realizada}
                                                </p>
                                              )}
                                              {turnoNaoRealizou && (
                                                <p className="text-xs text-red-700 ml-4 font-semibold mt-1">
                                                  Turno {turnoNaoRealizou} - {tecnicoNaoRealizou || supervisorNaoRealizou || 'N/A'}
                                                </p>
                                              )}
                                            </div>
                                          );
                              ...
                              {equip.anotacoes && (
                                <div className="bg-slate-50 border border-slate-200 rounded p-2 mt-3">
                                  <p className="text-sm font-semibold text-slate-900">OBS:</p>
                                  <p className="text-sm text-slate-700 whitespace-pre-wrap">{equip.anotacoes}</p>
                                </div>
                              )}
                              ...
                              <p className="text-sm font-bold text-blue-700 mt-2">STATUS: PARADO</p>
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        {/* Dialogs (manter iguais ao original) */}
        ...
        <Dialog open={!!equipamentoParaEditar} onOpenChange={(open) => !open && setEquipamentoParaEditar(null)}>
          <DialogContent className="max-w-[95vw] md:max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 md:space-y-4">
              ...
              <div>
                <Label className="text-sm">Tipo de Preventiva</Label>
                <Select
                  value={formEdicao.tipo_preventiva || 'total'}
                  onValueChange={(value) => setFormEdicao({ ...formEdicao, tipo_preventiva: value })}
                >
                  <SelectTrigger className="text-sm">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="total">Preventiva Total</SelectItem>
                    <SelectItem value="parcial">Preventiva Parcial</SelectItem>
                  </SelectContent>
                </Select>
              </div>
                  ...
                  try {
                    ...
                    await updateLiberacaoMutation.mutateAsync({
                      ...
                      data: {
                        ...
                        tipo_preventiva: formEdicao.tipo_preventiva,
